...
describe("RoleSelect Slide Tests", () => {
  ...
  it("should navigate to ParentAccountCredentials when Parent is selected", () => {
    cy.get('[data-testid="role-select-slide"]')
      .first()
      .within(() => {
        cy.contains("Parent").click({ force: true });
      });
    cy.get('[data-testid="role-select-continue-button"]').click();
    cy.get('[data-testid="parent-account-credentials-slide"]').should(
      "be.visible",
    );
  });
  ...
  it("should navigate to TeacherAccountCredentials when Teacher is selected", () => {
    ...
    cy.get('[data-testid="role-select-continue-button"]').click();
    cy.get('[data-testid="teacher-account-credentials-slide"]').should(
      "be.visible",
    );
  });
});
...
describe("ParentAccountCredentials Slide Tests", () => {
  ...
  it("ParentAccountCredentials form test", () => {
    ...
    cy.get('[data-testid="account-credentials-name-input"] input').type(
      "John Doe",
      { force: true },
    );
    cy.get('[data-testid="account-credentials-email-input"] input').type(
      "john@example.com",
      { force: true },
    );
    cy.get('[data-testid="account-credentials-password-input"] input').type(
      "password123",
      { force: true },
    );
    ...
    cy.get('[data-testid="language-select-continue-button"]').should(
      "be.visible",
    );
  });
});
