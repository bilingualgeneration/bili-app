...
describe("Extended Radio", () => {
  ...
  it("should render each option", () => {
    render(<ExtendedRadio name="field" control={control} options={options} />);
  });

  it("should add the default active class to a clicked component", () => {
    const { container } = render(
      <ExtendedRadio name="field" control={control} options={options} />,
    );
    const target: Element | null = container.querySelector(
      "[data-testId=extended-radio-component] *:nth-child(2)",
    );
